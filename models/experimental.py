...
def attempt_load(weights, map_location=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        attempt_download(w)
        # PyTorch >= 2.6 defaults weights_only=True which breaks loading legacy checkpoints.
        # Force weights_only=False for trusted local checkpoints, with fallback for older torch versions.
        try:
            ckpt = torch.load(w, map_location=map_location, weights_only=False)  # load (allow full unpickling)
        except TypeError:
